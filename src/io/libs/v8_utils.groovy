...
def buildCF(dir = '', uccode = ''){
    ...
    returnCode = cmd("vrunner compile --src \"${dir}\\src\\cf\" -c --ibconnection /S${server1c}/${database} --db-user \"Admin\" --db-pwd \"123\" --v8version \"8.3.23.1688\" --uccode \"${uccode}\" ")
    if(returnCode > 0){
...
def updatedb(uccode = ''){
    ...
    returnCode = cmd("vrunner updatedb --v1 --ibconnection /S${server1c}/${database} --db-user \"Admin\" --db-pwd \"123\" --v8version \"8.3.23.1688\" --uccode \"${uccode}\" ")
